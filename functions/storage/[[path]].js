...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!base) return jsonResp(500, url, { error: 'SUPABASE_URL 未配置' });
    let subpath = url.pathname.replace(/^\/storage/, '') || '/';
    if (!subpath.startsWith('/')) subpath = '/' + subpath;
    const target = base + '/storage' + subpath + url.search;
    ...
    try {
      return jsonResp(500, url, { error: 'PROXY_ERR: ' + (err && err.message) });
    } catch (e2) {
